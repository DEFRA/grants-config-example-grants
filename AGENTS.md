...
Grant configuration lives in `configurations/`, with one directory per grant. Service code and release helpers live in `src/` and `ci/`; Changesets are in `.changeset/`. Tests are colocated as `*.test.js`.
...
Use ES modules, 2-space indentation, single quotes, and no semicolons as enforced locally. Keep configuration directories named by grant slug and preserve existing JSON/YAML structure.
